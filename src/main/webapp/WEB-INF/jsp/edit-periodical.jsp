            ...
            <form class="form-horizontal" method="POST" action="${pageContext.request.contextPath}/controller?command=edit_periodical">
                ...
                <fieldset>
                        ...
                        <div class="col-md-4">
                            <select id="type" name="type" data-style="btn-primary" class="selectpicker form-control input-md show-tick" title="<fmt:message key="period.type"/>">
                                <c:forEach items="${periodicalTypes}" var="periodicalType">
                                    <option <c:if test="${periodicalType.id eq periodical.typeId}">selected</c:if> title="${periodicalType.name}" data-subtext="${periodicalType.name}">${periodicalType.id}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <input id="periodicity" name="periodicity" type="number" placeholder="<fmt:message key="period.periodicity"/>" class="form-control input-md" min="1" value="${periodical.periodicity}">
                        </div>
                    </div>
                            ...
                            <div class="col-md-4">
                                <select id="author" name="author" data-style="btn-danger" class="selectpicker form-control input-md show-tick" title="<fmt:message key="period.author"/>">
                                    <c:forEach items="${authors}" var="author">
                                        <option <c:if test="${author.id eq periodical.authorId}">selected</c:if> title="${author.fullName}" data-subtext="${author.fullName}">${author.id}</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
